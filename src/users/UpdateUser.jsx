...
export default function UpdateUser() {
  ...
  const handleSubmit = async event => {
    ...
    const user = input
    try {
      axios.put(`/api/users/${user._id}`, user).then(res => {
  ...
  return (
    ...
    <div className='max-w-xl mx-auto flex justify-center items-center'>
      ...
      <form className='flex flex-col shadow-lg p-10 gap-4 w-full' onSubmit={handleSubmit}>
        ...
        <div>
          <input placeholder="Email" type="email" name="email" onChange={handleInput} value={input.email} className='block w-full px-4 py-2 text-xl font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none' />
